...
def process_data():
    ...
    datasets = {}
    ds_out = nc.Dataset('%s/%s'%(processed_data_dir, 'c20_200510.nc'), 'w')
    try:
	...
	for variable in ['lat', 'lon', 'time', 'level']:
		...
		ds_out.variables[variable][:] = air.variables[variable][1092:1212:2]
	    elif variable == 'level':
...
def copy_variables(ds_in, ds_out, map_item, level_indices):
	...
	ds_out.variables[map_item['var_out']][:, :, :] = ds_in.variables[map_item['var_in']][1092:1212:2, 0]
	...
	for i, level_index in enumerate(level_indices):
	    ...
	    ds_out.variables[map_item['var_out']][:, i, :, :] = ds_in.variables[map_item['var_in']][1092:1212:2, level_index]
    else:
	ds_out.createVariable(map_item['var_out'], 'f4', ('time', 'lat', 'lon' ))
	print('out shape %s'%str(ds_out.variables[map_item['var_out']].shape))
	#ds_out.variables[map_item['var_out']][:, :, :] = ds_in.variables[map_item['var_in']][::2]
	ds_out.variables[map_item['var_out']][:, :, :] = ds_in.variables[map_item['var_in']][1092:1212:2]
    print('done')
